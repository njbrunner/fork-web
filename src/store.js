    ...
    actions: {
        login({commit}, data) {
            return new Promise((resolve, reject) => {
                commit('auth_request');
                axios({url: 'https://fork.herokuapp.com/auth/login', data: data, method: 'POST'})
                .then(response => {
        ...
        register({commit}, data) {
            return new Promise((resolve, reject) => {
                commit('auth_request');
                axios({url: 'https://fork.herokuapp.com/auth/signup', data: data, method: 'POST'})
                .then(response => {
                    // const chef_token = response.data.Data.chef_token
                    // localStorage.setItem('chef_token', chef_token)
                    // axios.defaults.headers.common['Authorization'] = 'Bearer ' + chef_token
        ...
        fetchRecipies({commit}) {
            return new Promise((resolve, reject) => {
                axios({url: 'https://fork.herokuapp.com/recipe/', method: 'GET'})
                .then(response => {
        ...
        createRecipe({commit}, data) {
            return new Promise((resolve, reject) => {
                console.log(data);
                axios({url: 'https://fork.herokuapp.com/recipe/', data: data, method: 'POST'})
                .then(response => {
                    resolve(response);
                })
                .catch(error => {
